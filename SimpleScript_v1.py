...
import sys                      # detects if running as a frozen PyInstaller exe
...
# True only inside a compiled exe, not when running the .py directly
if getattr(sys, "frozen", False):
    APP_DIR = os.path.dirname(sys.executable)   # folder next to the .exe - persists between runs
    BUNDLE_DIR = sys._MEIPASS                   # temp folder where bundled read-only assets live
else:
    APP_DIR = os.path.dirname(os.path.abspath(__file__))   # folder next to the script
    BUNDLE_DIR = APP_DIR

SAVE_DIR = os.path.join(APP_DIR, "sqnc")
os.makedirs(SAVE_DIR, exist_ok=True)            # create it if missing

# audio, path loaded by beep()
SOUND_PATH = os.path.join(BUNDLE_DIR, "assets", "sell20ct.mp3")
    ...
    def _bottom_bar(self, parent, back_cmd=None):
        ...
        if back_cmd:    # editor/saved view: show a "← back" link instead of "recorded"
            ...
            if has_files:
                ...
                # overstrike needs a Font object, not a widget option
...
# home actions ────────────────────────────────
    ...
    def _add_match(self, ora):                                  # logs a match timestamp
        ...
        trimmed = len(self.notif_matches) > 50                  # cap history at 50 entries
        if trimmed:
