...
SOUND_PATH = r"C:\Users\v404lerio\Desktop\sell20ct.mp3"
...
class SimpleScript(tk.Tk):
    ...
    def _go_to_tray(self):
        self.withdraw()
        if TRAY_AVAILABLE:
            img = Image.new("RGB", (64,64), color="#000000")
            draw = ImageDraw.Draw(img)
            draw.rectangle([20,20,44,44], fill="#555555")
            def show(icon, item):
                icon.stop()
    ...
    def _mkbtn(self, parent, label, cmd, dim=False, small=False):
        f = FONT_SML if small else FONT_BTN
        fg = C_DIM if dim else C_MID
        hb = "#111111" if dim else C_BORDER
    ...
    def _go_background(self): self.iconify()
    ...
    def _bottom_bar(self, parent, back_cmd=None):
        ...
        if back_cmd:
            ...
            if has_files:
                self._mklink(bottom_row, "recorded", self._open_saved_view).pack(side="right")
            else:
                tk.Label(bottom_row, text="recorded", font=FONT_SML,
                    bg=BG, fg=C_DIM, overstrike=True).pack(side="right")
        return bar
    ...
    def _build_home(self):
        ...
        # Bottom bar fissa prima del content
        self._bottom_bar(self.home)
        ...
        # Notifiche accumulate - Text widget readonly
        self.notif_frame = tk.Frame(content, bg=BG)
        ...
        self.notif_text.bind("<MouseWheel>",
            lambda e: self.notif_text.yview_scroll(int(-1*(e.delta/120)), "units"))
        ...
        self.status_lbl = tk.Label(content, text="— no trigger set",
            bg=BG, fg=C_DIM, font=FONT, anchor="w")
        self.status_lbl.pack(anchor="w", pady=(0,12))

        # Tasti principali
        btn_row = tk.Frame(content, bg=BG)
        btn_row.pack(anchor="w", pady=(0,14))
        self._mkbtn(btn_row, "set_trigger", self._open_editor).pack(side="left", padx=(0,8))
        self.btn_stop = self._mkbtn(btn_row, "stop_seek", self._do_stop)
    ...
    def _update_notif_height(self): pass

    def _add_match(self, ora):
        ...
        if len(self.notif_matches) > 50:
            self.notif_text.delete("end-2l", "end")
        self.notif_text.configure(state="disabled")
        self.notif_frame.pack(anchor="w", pady=(0,6))
        if len(self.notif_matches) > 5:
            self.notif_arrow.pack(pady=(2,0))
        else:
    ...
    def _open_editor(self):
        ...
        tk.Button(nav, text="−", font=FONT, bg=BG, fg=C_MID, relief="flat", bd=0,
            ...
            cursor="hand2", padx=6, pady=2,
            activebackground=BG, activeforeground=C_WHITE,
            command=self._e_del).pack(side="left")
        ...
        for r in range(7):
            for c in range(7):
                ...
                self.e_canvas.tag_bind(rect, "<Button-1>",
                    lambda e, i=idx: self._e_toggle(i))
                self.e_cells.append(rect)
        self._e_render()

        # Riga tasti: listen a sinistra, hamburger a destra
        btn_row = tk.Frame(content, bg=BG)
        btn_row.pack(fill="x", pady=(0,8))

        self._mkbtn(btn_row, "listen", self._do_listen).pack(side="left")

        # Hamburger a destra con bordo quadrato
        ...
        # Bottom bar con back a sinistra e sqnc.run a destra
    ...
    def _toggle_e_menu(self):
        if self.e_menu_visible: self._hide_e_menu()
            ...
            x = x - 20
            y = y + 160
            self.e_menu.place(x=x, y=y)
            self.e_menu.lift()
            self.e_menu_visible = True

    def _update_menu_state(self): pass
    ...
    def _e_paste_seq(self):
        try:
            ...
            if len(t) < 98:
                ...
                return
            n = min(len(t)//49, MAX)
            new_frames = [[int(c) for c in t[i*49:(i+1)*49]] for i in range(n)]
            # Inserisce dalla posizione corrente sovrascrivendo e aggiungendo
            pos = self.e_cur
    ...
    def _do_listen(self):
        ...
        if len(self.e_frames) < 2:
            ...
            return
        trigger_pattern = [list(f) for f in self.e_frames]
    ...
    def _open_saved_view(self):
        ...
        if not files:
            ...
            for fn in files:
                self._render_saved_entry(inner, fn, sc)

        self._bottom_bar(self.saved_view, back_cmd=self._close_saved_view)

    def _render_saved_entry(self, parent, fn, scroll_canvas):
        prefix = fn.split("_")[0]
        color = "#555555"
        ...
        arrow_btn.pack(side="left", padx=(0,4))

        tk.Label(hdr, text=fn, bg=BG, fg=color, font=FONT).pack(side="left")
        ...
        tk.Button(hdr, text="copy", font=FONT_SML,
            bg=BG, fg=C_DIM, relief="flat", bd=0, cursor="hand2",
            activebackground=BG, activeforeground=C_MID,
            command=copy_seq).pack(side="left", padx=(10,0))
        ...
        if prefix == "rcvd":
            ...
            try:
                ...
                for i in range(n):
                    if i % 5 == 0:
                        mini_row = tk.Frame(mini_container, bg=BG)
    ...
    def _del_saved(self, fn):
        path = os.path.join(SAVE_DIR, fn)
        if os.path.exists(path):
            os.remove(path)

    def _render_saved(self): pass
    ...
    def _check_match(self):
        if trigger_pattern is None or len(frame_window) < trigger_len:
            return False
        window = frame_window[-trigger_len:]
        return all(window[i][j] == trigger_pattern[i][j]
                   for i in range(trigger_len) for j in range(49))

    # ── RENDER ───────────────────────────────

    def _render_live(self, frame, flash=False):
        for i, rect in enumerate(self.cells):
            color = C_ON if (flash or frame[i]) else C_OFF
    ...
    def _tick(self):
        ...
        self._render_live(current_frame)

        if listening and not recording and self._check_match():
            if sound_on:
                threading.Thread(target=beep, daemon=True).start()
            self._render_live(current_frame, flash=True)
            ora = ts_display()
            self._add_match(ora)
            frame_window.clear()
            if record_on:
